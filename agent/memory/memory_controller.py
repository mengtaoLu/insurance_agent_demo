...
async def refresh_memory(chat_id:int,db):
    ## 先查询是否有记忆
    memory = get_memory_by_id(chat_id,db)

    if not memory:
        return await _init_memory(chat_id=chat_id,db=db)
    else :
        return await _update_memory(chat_id=chat_id,db=db,old_memory=memory)

async def _init_memory(chat_id:int,db):
    """初始化记忆读取用户对话id的信息"""
...
async def _update_memory(chat_id:int,db,old_memory:ChatMemoryRecord):
    """更新记忆"""
